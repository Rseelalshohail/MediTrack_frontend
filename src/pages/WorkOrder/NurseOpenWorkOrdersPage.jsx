...
export default function NurseOpenWorkOrdersPage() {
        ...
        async function fetchData() {
            try {
                ...
                const filtered = all.filter(order =>
                    order.created_by_display === user.username && order.status === 'open'
                )
                setOrders(filtered)
            } catch (err) {
                setError('Failed to load open work orders.')
            } finally {
                setLoading(false)
            }
        }
                    ...
                    <div className="p-6">
                        {loading ? (
                            ...
                            <div className="bg-gray-50 rounded-lg p-8 text-center">
                                ...
                                <p className="text-gray-500 text-lg">No open work orders found.</p>
                            </div>
                        ) : (
